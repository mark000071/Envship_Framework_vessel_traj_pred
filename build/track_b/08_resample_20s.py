...
def main() -> None:
    ...
    for input_path in list_primary_stage_csvs(args.input):
        ...
        # Preserve the input column schema even for all-NaN columns (else downstream stages
        # like 09/11/12 crash on KeyError for ship_type/heading/etc. in MMSI buckets without
        # static info).
        for col in df.columns:
            if col not in result.columns:
                result[col] = pd.NA
        result = result[list(df.columns) + [c for c in result.columns if c not in df.columns]]
